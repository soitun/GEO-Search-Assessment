...
**手动输入**：社区运营根据真实开发者采访反馈，使用 Markdown 文档（`manual-questions.md`），skill 自动解析并转换为结构化 JSON。

**路径一（主要）：论坛使用问题提取（使用阶段）**
...
优先级排序：手动 > 路径一（论坛）/ 路径二（Issue）> 多来源交叉 > 单来源
...
| 编号 | 问题 | 决策 |
|------|------|------|
| 3.1 | LLM 准确率判断标准 | 以官方源引用比例为主轴，五种现象分类 |
| 3.2 | 是否需要官方文档作为参考 | 需要：`content-labels.json` 人工预标注 |
| 3.3 | 评分 Prompt 模板 | 待设计 |
| 3.4 | 评分结果人工抽检校准 | **是**，分层抽检 20%，校准记录到 `scoring-calibration.md` |
